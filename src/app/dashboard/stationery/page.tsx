...
export default function StationeryPage() {
  ...
  const { data: stationeryItems, isLoading } = useCollection<StationeryItem>(stationeryItemsQuery);
  ...
  const handlePlaceOrder = async () => {
    ...
    try {
      ...
      const userOrderRef = doc(collection(db, 'voyagers', user.uid, 'stationeryOrders'));
      batch.set(userOrderRef, orderData);

      // 2. Add to denormalized collection for admin/supervisor view
      const adminOrderRef = doc(collection(db, 'allStationeryOrders'));
      ...
      toast({
        title: 'Order Placed!',
        description: 'Your stationery order has been successfully submitted.',
      });
      setCart([]);
    } catch (error) {
        ...
        <div className="grid gap-6 sm:grid-cols-2 lg:grid-cols-3">
          {isLoading ? (
            ...
            stationeryItems.map((item) => (
             ...
             <p className="text-muted-foreground col-span-full text-center">No stationery or gift items available at the moment.</p>
          )}
        </div>
      </div>
      <div>
