...
import type { CateringOrder, CateringItem } from '@/types';
...
export default function CateringAnalyticsPage() {
  const db = useFirestore();
  
  const ordersQuery = useMemoFirebase(() => db ? query(collection(db, 'allCateringOrders')) : null, [db]);
  const { data: orders, isLoading: ordersLoading } = useCollection<CateringOrder>(ordersQuery);
  
  const itemsQuery = useMemoFirebase(() => db ? query(collection(db, 'cateringItems')) : null, [db]);
  const { data: items, isLoading: itemsLoading } = useCollection<CateringItem>(itemsQuery);
  ...
  return (
    <div className="space-y-6">
      <CardHeader className="p-0">
        ...
        <CardDescription>
          Insights into catering performance and order trends.
        </CardDescription>
      </CardHeader>
