...
export default function CateringOrdersPage() {
    ...
    const ordersQuery = useMemoFirebase(
        () => (db && !authLoading) ? query(collection(db, 'allCateringOrders'), orderBy('orderDate', 'desc')) : null,
        [db, authLoading]
    );
    const { data: orders, isLoading: ordersLoading } = useCollection<CateringOrder>(ordersQuery);

    const handleStatusChange = async (order: CateringOrder, status: CateringOrder['status']) => {
        if (!db) return;
        const orderRef = doc(db, 'allCateringOrders', order.id);
        const userOrderRef = doc(db, 'voyagers', order.voyagerId, 'cateringOrders', order.originalOrderId!);
    ...
    const getStatusVariant = (status: CateringOrder['status']) => {
        switch (status) {
    ...
    return (
        <Card>
            <CardHeader>
                ...
                <CardDescription>View and manage all incoming catering orders from voyagers.</CardDescription>
            </CardHeader>
            <CardContent>
                 {isLoading ? (
                    <Table>
                                    ...
                                    <TableRow key={order.id}>
                                        ...
                                        <TableCell className="text-right">
                                            <DropdownMenu>
                                                ...
                                                <DropdownMenuContent>
                                                    {(['Pending', 'Preparing', 'Delivered', 'Cancelled'] as CateringOrder['status'][]).map((status) => (
